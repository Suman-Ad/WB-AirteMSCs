...
export default function AppContent() {
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(userRef, (snap) => {
            if (!snap.exists()) {
                forceLogout("Your account has been removed by Admin.");
                return;
            }
            ...
            if (latest.isActive === false) {
                forceLogout("Your account has been deactivated by Admin.");
                return;
            }
            ...
            for (const field of criticalFields) {
                if (latest[field] !== userData[field]) {
                    forceLogout("Your access permissions were updated. Please login again.");
                    return;
                }
            }
        });
