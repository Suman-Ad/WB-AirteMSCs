...
async function applyTemplateToMonth(siteId, selectedMonth, template) {
  ...
  days.forEach((day) => {
    ...
    const weekday = format(day, "EEE"); // Mon, Tue, Wed, Thu, Fri, Sat, Sun

    // if template contains this weekday → apply shifts
    if (template[weekday]) {
      batch.set(
        doc(db, "dutyRoster", `${siteId}_${iso}`),
        {
          siteId,
          date: iso,
          shifts: template[weekday], // { M:[], E:[], N:[] }
          autoGenerated: true,
          updatedAt: serverTimestamp(),
        },
        { merge: true }
      ...
      >
        ...
        >
          ...
          <button
            ...
            onClick={() => applyTemplateToMonth(selectedSite, selectedMonth, weekdayTemplate)}
          >
            Apply Template to This Month
          </button>
        </motion.div>
      </div>
    </div>
